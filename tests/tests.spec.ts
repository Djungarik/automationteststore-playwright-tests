...
test("buy the 1st featured product as a guest user with a standard shipping from the home page", async ({
  ...
  const postcode = guestUser.postcode;
  const shippingMethod = "Flat Shipping Rate";
  const paymentMethod = "Cash On Delivery";

  await pm
    .onCheckoutPage()
    .fillGuestForm(firstName, lastName, email, address1, city, zone, postcode);
  ...
  expect(shippingDetails.address).toContain(
    `${guestUser.address1} ${guestUser.city} ${guestUser.zone} ${guestUser.postcode} ${guestUser.country}`
  );
